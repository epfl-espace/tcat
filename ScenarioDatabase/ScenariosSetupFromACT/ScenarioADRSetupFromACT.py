...
class ScenarioADRSetupFromACT(ScenarioBaseSetupFromACT):
    ...
    def read_orbit_servicer_disposal_apogee(self, act_config_name):
        param_value = self.act_db_linker.get_bb_child_parameter_value( \
            act_config_name, BB_ID_SERVICER, BB_ID_EOL_STRATEGY, PARAM_ID_ORBIT_DISPOSAL_APOGEE)
    ...
    def read_orbit_servicer_disposal_perigee(self, act_config_name):
        param_value = self.act_db_linker.get_bb_child_parameter_value( \
            act_config_name, BB_ID_SERVICER, BB_ID_EOL_STRATEGY,PARAM_ID_ORBIT_DISPOSAL_PERIGEE)
        if self.act_db_linker.check_parameter_value(param_value):
    ...
    def read_orbit_servicer_disposal_inclination(self, act_config_name):
        param_value = self.act_db_linker.get_bb_child_parameter_value( \
            act_config_name, BB_ID_SERVICER, BB_ID_EOL_STRATEGY, PARAM_ID_ORBIT_DISPOSAL_INCLINATION)
        if self.act_db_linker.check_parameter_value(param_value):
    ...
    def read_orbit_sats_operational_apogee(self, act_config_name):
        param_value = self.act_db_linker.get_bb_parameter_value( \
            act_config_name, BB_ID_ADR_STRATEGY, PARAM_ID_ORBIT_DEBRIS_APOGEE)
        if self.act_db_linker.check_parameter_value(param_value):
    ...
    def read_orbit_sats_operational_perigee(self, act_config_name):
        param_value = self.act_db_linker.get_bb_parameter_value( \
            act_config_name, BB_ID_ADR_STRATEGY, PARAM_ID_ORBIT_DEBRIS_PERIGEE)
        if self.act_db_linker.check_parameter_value(param_value):
    ...
    def read_orbit_sats_operational_inclination(self, act_config_name):
        param_value = self.act_db_linker.get_bb_parameter_value( \
            act_config_name, BB_ID_ADR_STRATEGY, PARAM_ID_ORBIT_DEBRIS_INCLINATION)
        if self.act_db_linker.check_parameter_value(param_value):
    ...
    def read_servicer_config(self, act_config_name):
        servicer_name = self.get_servicer_name(act_config_name)
        ...
        if engine_name is not None:
            self.read_servicer_engine_parameters(act_config_name, servicer_name, engine_name)

    def get_servicer_name(self, act_config_name):
        config = self.act_db_linker.get_config(act_config_name)
        bb_servicer = self.act_db_linker.get_config_bb(config, BB_ID_SERVICER)
        return self.act_db_linker.get_bb_name(bb_servicer)
    ...
    def get_servicer_engine_name_from_child(self, act_config_name):
        name = self.act_db_linker.get_bb_child_name_filtered_type_id( \
            act_config_name, BB_ID_SERVICER, BB_ID_PROPULSION)
        if not self.act_db_linker.check_parameter_value(name, "servicer propulsion"):
            print("Servicer is missing a child propulsion block")
            return None
        return name

    def read_servicer_engine_parameters(self, act_config_name, servicer_name, engine_name):
        self.read_servicer_propulsion_thrust(act_config_name, servicer_name, engine_name)
        self.read_servicer_propulsion_isp(act_config_name, servicer_name, engine_name)
        self.read_servicer_propulsion_dry_mass(act_config_name, servicer_name, engine_name)

    def read_servicer_propulsion_thrust(self, act_config_name, servicer_name, engine_name):
        param_value = self.act_db_linker.get_bb_child_parameter_value( \
            act_config_name, BB_ID_SERVICER, BB_ID_PROPULSION, PARAM_ID_ENGINE_THRUST, servicer_name, engine_name)
        if self.act_db_linker.check_parameter_value(param_value):
            self.tcat_input_linker.servicer_prop_thrust = float(param_value*KN_N_UNIT_CONVERSION) # from ACT (kN) to TCAT (N)

    def read_servicer_propulsion_isp(self, act_config_name, servicer_name, engine_name):
        param_value = self.act_db_linker.get_bb_child_parameter_value( \
            act_config_name, BB_ID_SERVICER, BB_ID_PROPULSION, PARAM_ID_ENGINE_ISP, servicer_name, engine_name)
        if self.act_db_linker.check_parameter_value(param_value):
            self.tcat_input_linker.servicer_prop_isp = float(param_value)

    def read_servicer_propulsion_dry_mass(self, act_config_name, servicer_name, engine_name):
        param_value = self.act_db_linker.get_bb_child_parameter_value( \
            act_config_name, BB_ID_SERVICER, BB_ID_PROPULSION, PARAM_ID_DRY_MASS, servicer_name, engine_name)
        if self.act_db_linker.check_parameter_value(param_value):
