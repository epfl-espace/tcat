# TCAT
 ...
 > The key-generator will ask you where you want to save the key and if you want to protect it with a passphrase. Make sure your passphrase is empty (_if you want to create an auto deployment process later on, it will be easier without a passphrase_)
 ```shell
 ssh-keygen
 ```
 ...
 ```
 Create a folder with a virtual environment for python (_3.9_).
 ...
 pip install poliastro #when poliastro is in the requirements file a conflict occurs when installing
 ...
 ```
 In the file we configure the following options:
 ...
 sudo nginx -t
 ```
 When there are no errors, restart the Nginx process (_to load the new configuration_)
 ```shell
