...
class GenericPhase:
    ...
    def __str__(self):
        ...
        # build string
        if isinstance(self.spacecraft_snapshot, Scenario.Fleet_module.Servicer):
            return (str(self.ID)
                    + "\n\tEpoch: " + str(self.spacecraft_snapshot.current_orbit.epoch)
                    + "\n\tServicer: " + str(self.get_assigned_spacecraft().id)
                    + "\n\tAssociated Module: " + str(self.get_assigned_module().id)
                    + "\n\tTotal Duration : " + "{0:.1f}".format(duration_print)
                    + "\n\tServicer Orbit : " + orbit_string(self.spacecraft_snapshot.current_orbit)
                    + "\n\tServicer Current Mass : {0:.1f}".format(self.spacecraft_snapshot.get_current_mass())
                    + "\n\tServicer Wet Mass     : {0:.1f}".format(self.spacecraft_snapshot.get_wet_mass())
                    )
        else:
            return (str(self.ID)
