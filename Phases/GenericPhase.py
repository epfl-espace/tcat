...
class GenericPhase:
    ...
    def __str__(self):
        ...
        if self.servicer_snapshot:
            ...
            return (str(self.ID)
                    + "\n\tStarting Epoch: " + str(self.launcher_snapshot.previous_orbit.epoch)
                    + "\n\tEnding Epoch: " + str(self.launcher_snapshot.current_orbit.epoch)
                    + "\n\tLauncher: " + str(self.get_assigned_launcher().id)
                    + "\n\tAssociated Module: " + str(self.get_assigned_module().id)
                    + "\n\tTotal Duration: " + "{0:.1f}".format(duration_print)
                    + "\n\tInitial Orbit: " + orbit_string(self.launcher_snapshot.previous_orbit)
                    + "\n\tFinal Orbit: " + orbit_string(self.launcher_snapshot.current_orbit)
                    + "\n\tLauncher Mass After Phase: {0:.1f}".format(self.launcher_snapshot.get_current_mass())
                    + "\n\tFuel Mass After Phase: " + "{0:.1f}".format(self.launcher_snapshot.get_main_propulsion_module().current_propellant_mass)
