...
class ThermalModule(GenericModule):
    ...
    def compute_reference_power(self, plan):
        ...
        # otherwise apply adequate model if available
            ...
            self.reference_power = thermal_default_power * self.servicer.get_reference_power() / servicer_default_power
        else:
            raise TypeError('Missing Thermal power model for group ' + self.servicer.group + ' .')
    ...
    def compute_dry_mass(self, plan):
        ...
        # otherwise apply adequate model if available
            ...
            self.dry_mass = (thermal_default_mass * self.servicer.get_reference_power() / thermal_default_power
                             * (self.servicer.get_dry_mass(contingency=False) / servicer_default_mass) ** (1 / 3))
        elif self.servicer.group in ['ADR_servicers', 'tankers']:
            ...
            raise TypeError('Missing Thermal mass model for group ' + self.servicer.group + ' .')
    ...
    def get_recurring_cost(self):
        ...
        if self.recurring_cost_override is not None:
            ...
            raise TypeError('Missing Thermal recurring cost model for group ' + self.servicer.group + ' .')
    ...
    def get_non_recurring_cost(self):
        ...
        if self.non_recurring_cost_override is not None:
            ...
            raise TypeError('Missing Thermal non recurring cost model for group ' + self.servicer.group + ' .')
